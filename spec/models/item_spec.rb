  ...
  describe "商品の出品" do
    ...
    context "商品が出品できない場合" do
      it "ユーザー登録していないと出品できない" do
        @item.user = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("User must exist","User cant't be blank")
      end
      ...
      it "商品の状態の情報が「---」だと出品できない" do
        @item.situation_id = 0
        @item.valid?
        expect(@item.errors.full_messages).to include("Situation must be othert than 0")
      end
